...
pub const HOSTED_DISCOVERY_SNAPSHOT_URL: &str =
    "https://remingtonwilcox.github.io/mewsik/discovery/v1/snapshot.json";
...
pub(super) const LASTFM_CADENCE_SECS: u64 = 4 * 60 * 60;
pub(super) const YOUTUBE_CADENCE_SECS: u64 = 60 * 60;
...
pub struct SourceConfig {
    ...
    /// A public, credential-free snapshot generated by mewsik's scheduled
    /// GitHub workflow. `Default` leaves it off so isolated tests never make a
    /// network request; the real application enables the canonical endpoint in
    /// `from_env`.
    pub hosted_snapshot_url: Option<String>,
}
...
impl Default for SourceConfig {
    fn default() -> Self {
        Self {
            ...
            youtube_limit: 50,
            hosted_snapshot_url: None,
        }
    }
}

impl SourceConfig {
    /// Reads developer-only fallback keys and enables the public hosted
...
pub struct SourceFetchReport {
    ...
    /// Delivery state supplied by the shared snapshot service. Direct provider
    /// adapters leave this empty and retain the existing live/cached behavior.
    #[serde(default)]
    pub delivery_statuses: BTreeMap<String, SourceDeliveryStatus>,
    /// The snapshot publisher's next planned attempt. The feed runtime uses
    /// this to avoid retrying an unchanged CDN frame every minute.
    #[serde(default)]
    pub hosted_next_refresh_at: Option<i64>,
}
...
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq)]
pub struct SourceDeliveryStatus {
    pub state: SourceDeliveryState,
...
pub async fn fetch_all(
    ...
    let wants_hosted_lastfm = wants(LASTFM_SOURCE) && lastfm_key.is_none();
    let wants_hosted_youtube = wants(YOUTUBE_SOURCE) && youtube_key.is_none();
    ...
    let hosted_future = async {
        match (
            wants_hosted_lastfm || wants_hosted_youtube,
            config.hosted_snapshot_url.as_deref(),
        ) {
            (true, Some(url)) => Some(super::hosted::fetch_hosted_snapshot(client, url, now).await),
            _ => None,
        }
    };

    let (apple, listenbrainz, bandcamp, lastfm, youtube, hosted) = tokio::join!(
        apple_future,
        listenbrainz_future,
        bandcamp_future,
        lastfm_future,
    ...
    if let Some(result) = lastfm {
        record_result(&mut report, LASTFM_SOURCE, result);
    }
    if let Some(result) = youtube {
        record_result(&mut report, YOUTUBE_SOURCE, result);
    }

    let hosted_sources = [
        wants_hosted_lastfm.then_some(LASTFM_SOURCE),
        wants_hosted_youtube.then_some(YOUTUBE_SOURCE),
    ]
    .into_iter()
    .flatten()
    .collect::<HashSet<_>>();
    match hosted {
        Some(Ok(snapshot)) => {
            super::hosted::merge_hosted_snapshot(&mut report, snapshot, &hosted_sources)
        }
        Some(Err(error)) => {
            log::warn!("Shared discovery snapshot was unavailable: {error}");
            for source in &hosted_sources {
                report.failures.push(SourceFailure {
                    source: (*source).to_string(),
                    message: "The shared discovery service could not be loaded; saved data will be used when available"
                        .to_string(),
                });
            }
        }
    ...
    for source in hosted_sources {
        let accounted_for = report.batches.iter().any(|batch| batch.source == source)
            || report
                .failures
                .iter()
                .any(|failure| failure.source == source)
            || report.delivery_statuses.contains_key(source)
            || report.skipped.iter().any(|skipped| skipped == source);
        if !accounted_for {
...
/// Official Last.fm top-tracks API. The caller must supply a developer key;
/// `fetch_all` prefers the credential-free hosted snapshot when none is local.
