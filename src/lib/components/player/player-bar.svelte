...
>
	...
	<div class="flex min-w-0 flex-col items-center gap-1">
		<div class="flex items-center gap-1">
			<Toggle
				...
				aria-label="Shuffle"
				class="size-8 {player.state.is_shuffle ? 'text-primary' : ''}"
			>
				<Shuffle class="size-4" />
			</Toggle>
			<Button variant="ghost" size="icon" class="size-8" disabled={!hasCurrentItem} onclick={() => player.prev()}>
			...
			<Button variant="ghost" size="icon" class="size-8" disabled={!hasCurrentItem} onclick={() => player.next()}>
			...
			<Toggle
				...
				aria-label="Repeat"
				class="size-8 {player.state.repeat_mode !== 'off' ? 'text-primary' : ''}"
			>
		...
		{#if hasTimeline}
			<div style="display: grid; grid-template-columns: 40px 1fr 40px; align-items: center; gap: 6px; width: calc(100% + 40px); margin-left: -20px; margin-right: -20px;">
		...
		{:else if player.state.source === 'radio'}
			<div class="flex w-full items-center justify-center gap-2 text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
					{#if player.state.is_buffering}
		...
		{:else if player.state.is_buffering}
			<div class="flex w-full items-center justify-center gap-2 text-xs text-muted-foreground">
				<LoaderCircle class="size-3 animate-spin" />
		...
		{:else}
			<div class="flex w-full items-center justify-center text-xs text-muted-foreground">
	...
	<div class="flex shrink-0 items-center justify-end gap-2">
		<Button variant="ghost" size="icon" class="size-8" onclick={toggleMute}>
			{#if player.state.volume === 0}
